...
function parseJson(content: string): unknown { try { return JSON.parse(content) as unknown; } catch (error) { throw new Error(`Import file is not valid JSON: ${(error as Error).message}`); } }
