    ...
    <main className="main">
      ...
      <div ref={scrollRef} className="scroll" onScroll={handleThreadScroll}>
        {activeProject && !selectedChat && state
          ? <ProjectHome project={activeProject} state={state} models={providerModels} chats={chats.filter((chat) => chat.projectId === activeProject.id)} onSelectChat={selectChat} onNewChat={() => void createChat(activeProject.id, activeWorkspaceId)} onOpenContext={() => setDrawer("context")} refresh={refreshState} showToast={setToast} />
...
function ProjectHome(p: { project: Project; state: AppState; models: ProviderStatus["models"]; chats: Chat[]; onSelectChat: (id: string) => void; onNewChat: () => void; onOpenContext: () => void; refresh: () => Promise<void>; showToast: (s: string) => void }) {
