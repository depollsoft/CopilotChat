...
import type { AppState, Chat, ChatMessage, ImportDraft, McpServer, MessageAttachment, PermissionMode, Project, ProjectChatReference, ProjectChatSearchResult, ProjectReference, ProviderModel, ProviderStatus, Skill, Workspace } from "@copilotchat/shared";
...
function App(): React.ReactElement {
  ...
  const providerModels = provider.models.length > 0 ? provider.models : [{ id: fallbackModel, name: fallbackModel, supportsReasoningEffort: true, supportedReasoningEfforts: ["none", "low", "medium", "high", "xhigh"], defaultReasoningEffort: "medium", contextWindowTokens: 128000, maxPromptTokens: 128000 }];
  ...
  const effortChoices: ReasoningEffort[] = ["default", ...((selectedModelInfo?.supportedReasoningEfforts ?? []).filter((effort) => EFFORT_OPTIONS.includes(effort as ReasoningEffort)) as ReasoningEffort[])];
  const contextStatus = useMemo(() => buildContextStatus(messages, streamingText, draft, activeProject, activeWorkspace, selectedSkills, selectedModelInfo ?? null, providerModels), [messages, streamingText, draft, activeProject, activeWorkspace, selectedSkills, selectedModelInfo, providerModels]);
  useEffect(() => {
  ...
  useEffect(() => { if (selectedChat) { markChatSeen(selectedChat.id, selectedChat.updatedAt); setActiveProjectId(selectedChat.projectId); setActiveWorkspaceId(selectedChat.workspaceId); if (selectedChat.model) setSelectedModel(selectedChat.model); if (selectedChat.reasoningEffort && EFFORT_OPTIONS.includes(selectedChat.reasoningEffort)) setReasoningEffort(selectedChat.reasoningEffort); } }, [selectedChat]);
  ...
  async function refreshState(): Promise<void> { try { const next = await api<AppState>("/api/state", {}, apiToken); setLoginRequired(false); const selectedId = selectedChatIdRef.current; if (selectedId && ![...next.chats, ...next.archivedChats].some((chat) => chat.id === selectedId)) { selectedChatIdRef.current = null; setSelectedChatId(null); setMessages([]); setStreamingText(""); setStreamingActivities([]); setPendingInteractions([]); setPendingTurns([]); syncAppUrl(appPathForSelection(null, activeProjectId)); } setState(next); initializeSeenChatUpdates(next); } catch (e) { const message = toErr(e); if (message.includes("Login required") || message.includes("\"statusCode\":401")) { const status = await api<{mode:string;authenticated?:boolean;githubOAuthConfigured?:boolean}>("/api/auth/status").catch(() => null); if (status?.mode === "github" && !status.authenticated) { setLoginRequired(true); setError(status.githubOAuthConfigured ? null : "GitHub OAuth is not configured on this server."); return; } } if (message.includes("Unauthorized")) setDrawer("preferences"); setError(message); } }
  async function createChat(projectId = activeProjectId, workspaceId = activeWorkspaceId, options: { cleanup?: boolean; refresh?: boolean } = {}): Promise<Chat> { saveCurrentChatScroll(); activateLiveScroll(); const projectDefaultModel = projectId ? projects.find((project) => project.id === projectId)?.defaultModel : null; const chat = await api<Chat>("/api/chats", { method: "POST", body: { title: "New chat", projectId, workspaceId, model: projectDefaultModel ?? selectedModelInfo?.id ?? selectedModel, reasoningEffort } }, apiToken); syncAppUrl(appPathForSelection(chat.id, null)); selectedChatIdRef.current = chat.id; setSelectedChatId(chat.id); setState((current) => current ? { ...current, chats: [chat, ...current.chats.filter((item) => item.id !== chat.id)] } : current); setMessages([]); setStreamingText(""); setStreamingActivities([]); setPendingInteractions([]); setPendingTurns([]); setError(null); if (options.cleanup !== false) await cleanupAbandonedEmptyChats(chat.id); if (options.refresh !== false) await refreshState(); composerRef.current?.focus(); return chat; }
  async function renameChat(chat: Chat): Promise<void> { const title = await promptDialog({ title: "Rename chat", inputLabel: "Chat name", defaultValue: chat.title, placeholder: "Chat name", confirmLabel: "Rename" }); if (!title?.trim()) return; await api<Chat>(`/api/chats/${chat.id}`, { method: "PATCH", body: { title: title.trim() } }, apiToken); await refreshState(); }
  ...
  async function deleteChat(chat: Chat): Promise<void> { if (!(await confirmDialog({ title: "Delete chat?", message: `"${chat.title}" and its messages will be permanently deleted.`, confirmLabel: "Delete", tone: "danger" }))) return; await api<void>(`/api/chats/${chat.id}`, { method: "DELETE", raw: true }, apiToken); if (selectedChatId === chat.id) setSelectedChatId(null); await refreshState(); }
  ...
  async function createProjectFromSidebar(): Promise<void> { const name = await promptDialog({ title: "New project", inputLabel: "Project name", placeholder: "e.g. Marketing site", confirmLabel: "Create" }); if (!name?.trim()) return; const project = await api<Project>("/api/projects", { method: "POST", body: { name: name.trim(), description: "", instructions: "", memory: "" } }, apiToken); selectProject(project.id); await refreshState(); }
  async function renameProject(project: Project): Promise<void> { const name = await promptDialog({ title: "Rename project", inputLabel: "Project name", defaultValue: project.name, placeholder: "Project name", confirmLabel: "Rename" }); if (!name?.trim()) return; await api<Project>(`/api/projects/${project.id}`, { method: "PATCH", body: { name: name.trim() } }, apiToken); await refreshState(); }
  async function deleteProject(project: Project): Promise<void> { if (!(await confirmDialog({ title: "Delete project?", message: `"${project.name}" will be deleted. Its chats move back to General chats.`, confirmLabel: "Delete", tone: "danger" }))) return; await api<void>(`/api/projects/${project.id}`, { method: "DELETE", raw: true }, apiToken); if (activeProjectId === project.id) selectProject(null); await refreshState(); }
  function changeModel(model: string): void { setSelectedModel(model); if (selectedChatIdRef.current) void api<Chat>(`/api/chats/${selectedChatIdRef.current}`, { method: "PATCH", body: { model } }, apiToken).then(() => refreshState()).catch((e) => setError(toErr(e))); }
  ...
  async function sendMessage(content: string, options: { chat?: Chat; skillIds?: string[]; attachments?: MessageAttachment[] } = {}): Promise<void> { const trimmed = content.trim(); const attachments = options.attachments ?? []; if ((!trimmed && attachments.length === 0) || busy) return; activateLiveScroll(); setBusy(true); setError(null); setStreamingActivities([]); setPendingInteractions([]); setPendingTurns([]); let chat = options.chat ?? selectedChat; if (!chat) chat = await createChat(activeProjectId, activeWorkspaceId, { cleanup: false, refresh: false }); setDraft(""); setMessages((m) => [...m, { id: `optimistic-${Date.now()}`, chatId: chat.id, role: "user", content: trimmed, provider: null, metadata: attachments.length ? { attachments } : {}, createdAt: new Date().toISOString() }]); const controller = new AbortController(); abortRef.current = controller; await streamChatResponse(chat.id, `/api/chats/${chat.id}/messages`, turnBody(trimmed, chat, options.skillIds, attachments), controller); }
  async function sendWhileBusy(mode: "steer" | "queue", content: string): Promise<void> { const trimmed = content.trim(); const chat = selectedChat; if (!trimmed || !chat || !busy) return; setDraft(""); const pending = await api<PendingTurn>(`/api/chats/${chat.id}/active-response/input`, { method: "POST", body: { ...turnBody(trimmed, chat), mode } }, apiToken); setPendingTurns((current) => upsertPendingTurn(current, pending)); }
  async function editAndContinue(messageId: string, content: string): Promise<void> { const trimmed = content.trim(); if (!selectedChat || !trimmed || busy) return; const index = messages.findIndex((message) => message.id === messageId); if (index < 0) return; activateLiveScroll(); setEditingMessage(null); setBusy(true); setError(null); setStreamingText(""); setStreamingActivities([]); setPendingInteractions([]); setPendingTurns([]); setMessages(messages.slice(0, index + 1).map((message) => message.id === messageId ? { ...message, content: trimmed, metadata: { ...message.metadata, editedAt: new Date().toISOString() } } : message)); const controller = new AbortController(); abortRef.current = controller; await streamChatResponse(selectedChat.id, `/api/chats/${selectedChat.id}/messages/${messageId}/edit`, { content: trimmed, skillIds: selectedSkillIds, model: selectedModel, reasoningEffort, permissionMode }, controller); }
  async function retryResponse(messageId: string): Promise<void> { if (!selectedChat || busy) return; const index = messages.findIndex((message) => message.id === messageId); if (index < 0) return; activateLiveScroll(); setBusy(true); setError(null); setStreamingText(""); setStreamingActivities([]); setPendingInteractions([]); setPendingTurns([]); setMessages(messages.slice(0, index)); const controller = new AbortController(); abortRef.current = controller; await streamChatResponse(selectedChat.id, `/api/chats/${selectedChat.id}/messages/${messageId}/retry`, { skillIds: selectedSkillIds, model: selectedModel, reasoningEffort, permissionMode }, controller); }
  ...
  function turnBody(content: string, chat: Chat, skillIds = selectedSkillIds, attachments: MessageAttachment[] = []): Record<string, unknown> { return { content, attachments, projectId: chat.projectId, workspaceId: chat.workspaceId ?? activeWorkspaceId, skillIds, model: selectedModel, reasoningEffort, permissionMode }; }
  ...
  async function clearAllData(): Promise<void> {
    if (!(await confirmDialog({ title: "Clear all local data?", message: "This deletes chats, projects, artifacts, skills, MCP servers, workspaces, tool history, and isolated chat workspaces. Account and auth setup are kept.", confirmLabel: "Clear data", tone: "danger" }))) return;
...
function Header(p: { chatTitle: string; projectName: string | null; chatFavorite: boolean | null; projectFavorite: boolean | null; provider: ProviderStatus; model: string; models: ProviderStatus["models"]; effort: ReasoningEffort; effortChoices: ReasoningEffort[]; context: ContextStatus; busy: boolean; onModelChange: (v: string) => void; onEffortChange: (v: ReasoningEffort) => void; onToggleSidebar: () => void; onToggleChatFavorite?: () => void; onToggleProjectFavorite?: () => void; onRenameChat?: () => void; onArchiveChat?: () => void; onDeleteChat?: () => void; onRenameProject?: () => void; onDeleteProject?: () => void; onOpenTab: (t: Tab) => void; onNewChat: () => void }) {
  ...
  return <header className={`header${p.busy ? " thinking" : ""}`}>
    ...
    <div className="header-controls">
      <label className="header-select"><span>Model</span><select aria-label="Model" value={p.model} onChange={(e) => p.onModelChange(e.target.value)}>{p.models.map((model) => <option key={model.id} value={model.id}>{model.name || model.id}</option>)}</select></label>
      <label className="header-select effort"><span>Effort</span><select aria-label="Reasoning effort" value={p.effort} onChange={(e) => p.onEffortChange(e.target.value as ReasoningEffort)}>{p.effortChoices.map((effort) => <option key={effort} value={effort}>{effortLabel(effort)}</option>)}</select></label>
      <div ref={contextRef} className="context-ring-wrap"><button type="button" className={`context-ring ${p.context.state}`} title={p.context.detail} aria-label={`Context: ${p.context.detail}`} aria-expanded={showContext} aria-controls="context-details" style={{ background: contextRingBackground(p.context) }} onClick={() => setShowContext((value) => !value)}><span>{contextRingLabel(p.context)}</span></button>{showContext ? <div id="context-details" className="context-popover" role="status"><strong>Context</strong><span>{p.context.detail}</span><small>{p.context.label}</small></div> : null}</div>
    </div>
...
const Composer = React.forwardRef<ComposerHandle, { busy: boolean; project: Project | null; projects: Project[]; workspace: Workspace | null; workspaces: Workspace[]; skills: Skill[]; selectedSkills: Skill[]; selectedSkillIds: string[]; permissionMode: PermissionMode; setPermissionMode: (mode: PermissionMode) => void; onDraftPreviewChange: (v: string) => void; onSubmit: (content: string, attachments: MessageAttachment[]) => void; onSteer: (content: string) => void; onQueue: (content: string) => void; onStop: () => void; onOpenTab: (t: Tab) => void; onSelectProject: (id: string | null) => void; onSelectWorkspace: (id: string | null) => void; onSelectSkills: (ids: string[]) => void }>(function Composer(p, ref) {
  ...
  function submitContent(handler: (content: string) => void): void { const content = value.trim(); if (!content) return; clearValue(); handler(content); }
...
function ComposerPickerContent(p: { picker: ComposerPicker; projects: Project[]; activeProjectId: string | null; workspaces: Workspace[]; activeWorkspaceId: string | null; skills: Skill[]; selectedSkillIds: string[]; permissionMode: PermissionMode; selectedSkillCount: number; activeProjectName: string | null; activeWorkspaceName: string | null; onAttach: () => void; onOpenPicker: (picker: ComposerPicker) => void; onSelectProject: (id: string | null) => void; onSelectWorkspace: (id: string | null) => void; onToggleSkill: (id: string) => void; onPermissionMode: (mode: PermissionMode) => void; onManage: (tab: Tab) => void }) {
  if (p.picker === "menu") return <><div className="picker-head"><strong>Composer options</strong></div><div className="picker-list"><button type="button" className="picker-option" onClick={p.onAttach}><span>Attach files or images</span><small>Upload files, images, screenshots, or paste images into the composer.</small></button><button type="button" className="picker-option" onClick={() => p.onOpenPicker("project")}><span>Project</span><small>{p.activeProjectName ?? "General chat, no project context"}</small></button><button type="button" className="picker-option" onClick={() => p.onOpenPicker("skills")}><span>Skills</span><small>{p.selectedSkillCount ? `${p.selectedSkillCount} selected for the next turn` : "Choose reusable behavior for the next turn"}</small></button><button type="button" className="picker-option" onClick={() => p.onOpenPicker("workspace")}><span>Workspace</span><small>{p.activeWorkspaceName ?? "No local folder attached"}</small></button><button type="button" className="picker-option" onClick={() => p.onOpenPicker("permissions")}><span>Tool permissions</span><small>{p.permissionMode === "yolo" ? "YOLO mode is on" : "Ask before tool use"}</small></button></div></>;
...
function ProjectChatReferences(p:{project:Project;state:AppState;refresh:()=>Promise<void>;showToast:(s:string)=>void}) {
  ...
  return <div className="project-context-block"><strong>Referenced chat context</strong><div className="row"><input placeholder="Search this project's chats" value={query} onChange={(e)=>setQuery(e.target.value)}/><button className="btn btn-sm" disabled={!query.trim()} onClick={()=>void search()}>Search</button></div>{results.map((result)=><div className="mini-card" key={result.messageId}><strong>{result.title}</strong><p>{result.excerpt}</p><button className="btn btn-sm" onClick={()=>void pin(result)}>Reference</button></div>)}{refs.map((reference)=><div className="mini-card" key={reference.id}><strong>{reference.title}</strong><p>{reference.excerpt}</p><button className="btn btn-sm btn-danger" onClick={async()=>{if(!(await confirmDialog({title:"Remove reference?",message:`"${reference.title}" will no longer be referenced in this project.`,confirmLabel:"Remove",tone:"danger"})))return; await api<void>(`/api/project-chat-references/${reference.id}`,{method:"DELETE",raw:true}); await p.refresh();}}>Remove</button></div>)}</div>;
}
function SkillsPanel(p: { state: AppState; selectedSkillIds: string[]; setSelectedSkillIds: (ids: string[]) => void; refresh: () => Promise<void>; showToast: (s:string)=>void }) { const { confirm: confirmDialog } = useDialog(); const [show,setShow]=useState(false),[name,setName]=useState(""),[desc,setDesc]=useState(""),[ins,setIns]=useState(""),[rules,setRules]=useState(""),[perms,setPerms]=useState<string[]>([]); function toggle(id:string){p.setSelectedSkillIds(p.selectedSkillIds.includes(id)?p.selectedSkillIds.filter(x=>x!==id):[...p.selectedSkillIds,id]);} async function create(){if(!name.trim()||!ins.trim())return; await api<Skill>("/api/skills",{method:"POST",body:{manifest:{id:slugify(name),name,description:desc||`${name} workflow`,version:"1.0.0",instructions:ins,prompts:[],workflow:[],artifactTemplates:[],mcpDependencies:[],toolDependencies:[],activationRules:parseRules(rules),permissions:perms}}}); setShow(false); setName(""); setDesc(""); setIns(""); setRules(""); setPerms([]); await p.refresh();} return <><div className="row"><p className="section-help">Installed skills auto-trigger from activation rules or when you mention them by name. Click a skill to explicitly include it in the next turn.</p><button className="btn btn-sm" onClick={()=>setShow(!show)}>{show?"Cancel":"New skill"}</button></div>{show?<form className="card" onSubmit={(e)=>{e.preventDefault();void create();}}><input placeholder="Code review" value={name} onChange={e=>setName(e.target.value)}/><input placeholder="A short summary" value={desc} onChange={e=>setDesc(e.target.value)}/><textarea rows={4} placeholder="Tell the assistant how to act when this skill is on." value={ins} onChange={e=>setIns(e.target.value)}/><textarea rows={3} placeholder="Activation rules, one per line, e.g. User asks for a review" value={rules} onChange={e=>setRules(e.target.value)}/>{PERMISSIONS.map(pm=><label key={pm} className="checkbox-row"><input type="checkbox" checked={perms.includes(pm)} onChange={()=>setPerms(perms.includes(pm)?perms.filter(x=>x!==pm):[...perms,pm])}/>{pm}</label>)}<button className="btn btn-primary" disabled={!name.trim()||!ins.trim()}>Install skill</button></form>:null}<div className="list">{p.state.skills.map(skill=><div key={skill.id} className={`toggle-card${p.selectedSkillIds.includes(skill.id)?" on":""}`}><button className="toggle-card-main" disabled={!skill.enabled} onClick={()=>toggle(skill.id)}><div className="toggle-card-h"><strong>{skill.manifest.name}</strong><span className="toggle-card-check"><IconCheck width={14}/></span></div><span>{skill.manifest.description}</span></button><div className="card-foot"><span className="tag">{skill.builtIn?"Built-in":"Custom"}</span><span className={`tag${skill.enabled?" success":" warning"}`}>{skill.enabled?"installed":"disabled"}</span>{p.selectedSkillIds.includes(skill.id)?<span className="tag success">explicit next turn</span>:null}{skill.manifest.activationRules.map(rule=><span key={rule} className="tag">{rule}</span>)}{skill.manifest.permissions.map(pm=><span key={pm} className="tag">{pm}</span>)}</div><div className="card-actions"><button className="btn btn-sm" onClick={async()=>{await api<Skill>(`/api/skills/${skill.id}`,{method:"PATCH",body:{enabled:!skill.enabled}}); await p.refresh();}}>{skill.enabled?"Disable":"Install"}</button>{!skill.builtIn?<button className="btn btn-sm btn-danger" onClick={async()=>{if(!(await confirmDialog({title:"Delete skill?",message:`"${skill.manifest.name}" will be removed.`,confirmLabel:"Delete",tone:"danger"})))return; await api<void>(`/api/skills/${skill.id}`,{method:"DELETE",raw:true}); await p.refresh();}}>Delete</button>:null}</div></div>)}</div></>; }
function McpPanel(p:{servers:McpServer[];refresh:()=>Promise<void>;showToast:(s:string)=>void}){const { confirm: confirmDialog } = useDialog(); const[transport,setTransport]=useState<"stdio"|"http"|"sse">("stdio"),[name,setName]=useState(""),[command,setCommand]=useState(""),[url,setUrl]=useState(""),[tools,setTools]=useState("*"); async function add(){if(!name.trim())return; await api<McpServer>("/api/mcp-servers",{method:"POST",body:{name,transport,command:transport==="stdio"?command:null,args:[],url:transport==="stdio"?null:url,tools:parseTools(tools),enabled:true}}); setName(""); setCommand(""); setUrl(""); await p.refresh();} return <><p className="section-help">Configure MCP servers and allowed tools.</p><form className="card" onSubmit={e=>{e.preventDefault();void add();}}><input placeholder="github" value={name} onChange={e=>setName(e.target.value)}/><label>Transport</label><select value={transport} onChange={e=>setTransport(e.target.value as "stdio"|"http"|"sse")}><option value="stdio">Local (stdio)</option><option value="http">HTTP</option><option value="sse">SSE</option></select>{transport==="stdio"?<input placeholder="npx -y @some/mcp-server" value={command} onChange={e=>setCommand(e.target.value)}/>:<input placeholder="https://api.example.com/mcp" value={url} onChange={e=>setUrl(e.target.value)}/>}<input placeholder="tool-a, tool-b, or *" value={tools} onChange={e=>setTools(e.target.value)}/><button className="btn btn-primary">Add MCP server</button></form>{p.servers.map(server=><div className="card" key={server.id}><div className="card-h"><strong>{server.name}</strong><span className={`tag${server.enabled?" success":" warning"}`}>{server.transport} · {server.enabled?"enabled":"disabled"}</span></div><code>{server.transport==="stdio"?server.command:server.url}</code><div className="card-foot">{(server.tools.length?server.tools:["no tools exposed"]).map(t=><span key={t} className="tag">{t}</span>)}</div><div className="card-actions"><button className="btn btn-sm" onClick={async()=>{await api<McpServer>(`/api/mcp-servers/${server.id}`,{method:"PATCH",body:{enabled:!server.enabled}}); await p.refresh();}}>{server.enabled?"Disable":"Enable"}</button><button className="btn btn-sm btn-danger" onClick={async()=>{if(!(await confirmDialog({title:"Remove MCP server?",message:`"${server.name}" will be removed from CopilotChat.`,confirmLabel:"Remove",tone:"danger"})))return; await api<void>(`/api/mcp-servers/${server.id}`,{method:"DELETE",raw:true}); await p.refresh();}}>Remove</button></div></div>)}</>;}
function WorkspacesPanel(p:{workspaces:Workspace[];activeWorkspaceId:string|null;setActiveWorkspaceId:(id:string|null)=>void;refresh:()=>Promise<void>;showToast:(s:string)=>void}){const[name,setName]=useState(""),[rootPath,setRootPath]=useState(""); async function reg(){if(!name||!rootPath)return; await api<Workspace>("/api/workspaces",{method:"POST",body:{name,rootPath}}); setName(""); setRootPath(""); await p.refresh();} return <><p className="section-help">Connect local folders for cowork mode.</p><form className="card" onSubmit={e=>{e.preventDefault();void reg();}}><input placeholder="My repo" value={name} onChange={e=>setName(e.target.value)}/><input placeholder="/Users/you/Code/project" value={rootPath} onChange={e=>setRootPath(e.target.value)}/><button className="btn btn-primary">Register workspace</button></form>{p.workspaces.map(w=><WorkspaceCard key={w.id} workspace={w} active={p.activeWorkspaceId===w.id} setActiveWorkspaceId={p.setActiveWorkspaceId} refresh={p.refresh}/>)}</>;}
function WorkspaceCard(p:{workspace:Workspace;active:boolean;setActiveWorkspaceId:(id:string|null)=>void;refresh:()=>Promise<void>}){const { confirm: confirmDialog } = useDialog(); const[command,setCommand]=useState("ls"),[output,setOutput]=useState(""); async function run(){const r=await api<{output:{stdout?:string;stderr?:string};error:string|null}>(`/api/workspaces/${p.workspace.id}/commands`,{method:"POST",body:{command}}); setOutput([r.output.stdout,r.output.stderr,r.error].filter(Boolean).join("\n")||"(no output)");} return <div className="card"><div className="card-h"><strong>{p.workspace.name}</strong><span className={`tag${p.active?" success":""}`}>{p.active?"active":p.workspace.enabled?"enabled":"disabled"}</span></div><code>{p.workspace.rootPath}</code><div className="card-actions"><button className="btn btn-sm" disabled={!p.workspace.enabled} onClick={()=>p.setActiveWorkspaceId(p.active?null:p.workspace.id)}>{p.active?"Stop using":"Use in chat"}</button><button className="btn btn-sm" onClick={async()=>{await api<Workspace>(`/api/workspaces/${p.workspace.id}`,{method:"PATCH",body:{enabled:!p.workspace.enabled}}); await p.refresh();}}>{p.workspace.enabled?"Disable":"Enable"}</button><button className="btn btn-sm btn-danger" onClick={async()=>{if(!(await confirmDialog({title:"Remove workspace?",message:`"${p.workspace.name}" will be unregistered. Files on disk are not deleted.`,confirmLabel:"Remove",tone:"danger"})))return; await api<void>(`/api/workspaces/${p.workspace.id}`,{method:"DELETE",raw:true}); await p.refresh();}}>Remove</button></div><form className="row" onSubmit={e=>{e.preventDefault();void run();}}><input placeholder="Command" value={command} onChange={e=>setCommand(e.target.value)}/><button className="btn btn-sm" disabled={!p.workspace.enabled}>Run</button></form>{output?<pre className="terminal">{output}</pre>:null}</div>;}
...
type DialogOptions = { title: string; message?: string; confirmLabel?: string; cancelLabel?: string; tone?: DialogTone; defaultValue?: string; placeholder?: string; inputLabel?: string };
type DialogState = DialogOptions & { kind: "confirm" | "prompt"; id: number };
type DialogApi = { confirm: (options: DialogOptions) => Promise<boolean>; prompt: (options: DialogOptions) => Promise<string | null> };
const DialogContext = React.createContext<DialogApi | null>(null);
function DialogProvider({ children }: { children: React.ReactNode }): React.ReactElement {
  const [dialog, setDialog] = useState<DialogState | null>(null);
  const resolveRef = useRef<((value: string | null) => void) | null>(null);
  const idRef = useRef(0);
  const open = useCallback((kind: "confirm" | "prompt", options: DialogOptions) => new Promise<string | null>((resolve) => {
    resolveRef.current?.(null);
    resolveRef.current = resolve;
    setDialog({ ...options, kind, id: (idRef.current += 1) });
  }), []);
  ...
  return <DialogContext.Provider value={api}>{children}{dialog ? <AppDialog key={dialog.id} dialog={dialog} onResolve={settle} /> : null}</DialogContext.Provider>;
}
...
function AppDialog({ dialog, onResolve }: { dialog: DialogState; onResolve: (value: string | null) => void }): React.ReactElement {
  ...
  const inputId = useId();
  const confirmDisabled = isPrompt && value.trim().length === 0;
  useEffect(() => {
    const previous = document.activeElement as HTMLElement | null;
    const target = isPrompt ? inputRef.current : tone === "danger" ? cancelRef.current : confirmRef.current;
    target?.focus();
    inputRef.current?.select();
    return () => previous?.focus?.();
  }, [isPrompt, tone]);
  function submit(): void { if (confirmDisabled) return; onResolve(isPrompt ? value.trim() : ""); }
  ...
  return <><div className="modal-scrim app-dialog-scrim" onClick={() => onResolve(null)} /><div ref={panelRef} className={`app-dialog${tone === "danger" ? " is-danger" : ""}`} role={isPrompt ? "dialog" : "alertdialog"} aria-modal="true" aria-labelledby={titleId} aria-describedby={dialog.message ? messageId : undefined} onKeyDown={keyDown}><div className="app-dialog-head"><h2 id={titleId}>{dialog.title}</h2>{dialog.message ? <p id={messageId}>{dialog.message}</p> : null}</div>{isPrompt ? <div className="app-dialog-body"><label htmlFor={inputId}>{dialog.inputLabel ?? "Name"}</label><input id={inputId} ref={inputRef} value={value} placeholder={dialog.placeholder} onChange={(e) => setValue(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") { e.preventDefault(); submit(); } }} /></div> : null}<div className="app-dialog-actions"><button ref={cancelRef} type="button" className="btn" onClick={() => onResolve(null)}>{dialog.cancelLabel ?? "Cancel"}</button><button ref={confirmRef} type="button" className={`btn ${tone === "danger" ? "btn-danger-solid" : "btn-primary"}`} disabled={confirmDisabled} onClick={submit}>{dialog.confirmLabel ?? (isPrompt ? "Save" : "Confirm")}</button></div></div></>;
}
...
async function api<T>(url:string,init:{method?:string;body?:unknown;raw?:boolean}={},token=localStorage.getItem(API_TOKEN_KEY)??""):Promise<T>{const method=init.method??"GET";const headers:Record<string,string>={};if(init.body!==undefined)headers["Content-Type"]="application/json";if(["POST","PATCH","DELETE"].includes(method))headers["X-CopilotChat-CSRF"]="1";if(token)headers.Authorization=`Bearer ${token}`;const res=await fetch(url,{method,headers:Object.keys(headers).length?headers:undefined,body:init.body!==undefined?JSON.stringify(init.body):undefined});if(!res.ok)throw new Error(await res.text());return init.raw?undefined as T:await res.json() as T;}
async function* streamSse(url:string,body:unknown,signal:AbortSignal,token:string,method="POST"):AsyncIterable<SseEvent>{const headers:Record<string,string>={};if(body!==undefined)headers["Content-Type"]="application/json";if(["POST","PATCH","DELETE"].includes(method))headers["X-CopilotChat-CSRF"]="1";if(token)headers.Authorization=`Bearer ${token}`;const res=await fetch(url,{method,headers:Object.keys(headers).length?headers:undefined,body:body!==undefined?JSON.stringify(body):undefined,signal});if(!res.ok||!res.body)throw new Error(await res.text());const reader=res.body.getReader();const dec=new TextDecoder();let buf="";while(true){const{done,value}=await reader.read();if(done)break;buf+=dec.decode(value,{stream:true});const parts=buf.split("\n\n");buf=parts.pop()??"";for(const part of parts){const ev=parseSse(part);if(ev)yield ev;}}}
...
function contextRingBackground(status: ContextStatus): string { const percent = status.limitTokens ? Math.min(100, (status.estimatedTokens / status.limitTokens) * 100) : status.percent ?? 0; const visualPercent = status.estimatedTokens > 0 ? Math.max(2, percent) : percent; const degrees = Math.max(0, Math.min(100, visualPercent)) * 3.6; const color = status.state === "full" ? "var(--danger)" : status.state === "warn" ? "#d97706" : status.state === "unknown" ? "var(--border-strong)" : "var(--accent)"; return `conic-gradient(${color} ${degrees}deg, var(--surface-muted) 0deg)`; }
...
function buildContextStatus(messages:ChatMessage[],streamingText:string,draft:string,project:Project|null,workspace:Workspace|null,skills:Skill[],model:ProviderModel|null,models:ProviderModel[]):ContextStatus{const contextText=[...messages.map(m=>m.content),streamingText,draft,project?.instructions??"",workspace?.rootPath??"",...skills.map(s=>`${s.manifest.name}\n${s.manifest.description}\n${s.manifest.instructions}`)].filter(Boolean).join("\n\n");const estimatedTokens=Math.max(0,Math.ceil(contextText.length/4));const displayTokens=estimatedTokens===0?0:Math.max(1000,estimatedTokens);const limitTokens=model?.maxPromptTokens??model?.contextWindowTokens??largestContext(models);if(!limitTokens)return{estimatedTokens,limitTokens:null,percent:null,label:`${formatTokens(displayTokens)} used`,detail:`Estimated ${estimatedTokens.toLocaleString()} tokens. Context limit is not reported for this model.`,state:"unknown"};const actualPercent=(estimatedTokens/limitTokens)*100;const percent=Math.min(100,Math.round(actualPercent));const state=percent>=85?"full":percent>=65?"warn":"ok";return{estimatedTokens,limitTokens,percent,label:`${formatTokens(displayTokens)} / ${formatTokens(limitTokens)}`,detail:`Estimated ${estimatedTokens.toLocaleString()} of ${limitTokens.toLocaleString()} context tokens used (${formatPercent(actualPercent)}).`,state};}
function largestContext(models:ProviderModel[]):number|null{return models.map(m=>m.maxPromptTokens??m.contextWindowTokens??0).filter(v=>v>0).sort((a,b)=>b-a)[0]??null;}
...
const root=document.getElementById("root"); if(!root) throw new Error("Missing root"); createRoot(root).render(<React.StrictMode><DialogProvider><App/></DialogProvider></React.StrictMode>);
