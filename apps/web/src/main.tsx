...
function App(): React.ReactElement {
  ...
  useEffect(() => {
    if (busy) { respondedRef.current = true; setLiveStatus("Copilot is responding."); return; }
  ...
  return <div className={`app${sidebarCollapsed ? " sidebar-collapsed" : ""}`}>
    ...
    <main className="main">
      ...
      <h1 className="sr-only">{activeProject && !selectedChat ? `Project: ${activeProject.name}` : selectedChat?.title ?? "New conversation"}</h1>
      ...
      <div ref={scrollRef} className="scroll" onScroll={handleThreadScroll}>
        {activeProject && !selectedChat && state
          ...
          : messages.length === 0 && !streamingText && streamingActivities.length === 0 && pendingTurns.length === 0 && pendingInteractions.length === 0
            ? <Welcome userName={state?.owner.displayName ?? state?.owner.login ?? ""} project={activeProject} firstRun={chats.length === 0} onPrompt={(p) => { setDraft(p); composerRef.current?.setValue(p); composerRef.current?.focus(); }} />
...
function useModalDialog<T extends HTMLElement>(active: boolean, onClose: () => void): React.RefObject<T | null> {
  ...
  useEffect(() => {
    ...
    window.requestAnimationFrame(() => { const items = focusable(); (items[0] ?? ref.current)?.focus(); });
    function keyDown(event: KeyboardEvent): void {
