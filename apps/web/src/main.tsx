...
function App(): React.ReactElement {
  ...
  async function sendMessage(content: string, options: { chat?: Chat; skillIds?: string[]; attachments?: MessageAttachment[] } = {}, onAccepted?: () => void): Promise<void> { const trimmed = content.trim(); const attachments = options.attachments ?? []; if ((!trimmed && attachments.length === 0) || busy) return; activateLiveScroll(); setBusy(true); setError(null); setStreamingActivities([]); setPendingInteractions([]); setPendingTurns([]); try { let chat = options.chat ?? selectedChat; const useChatSettings = Boolean(options.chat) || !chat; if (!chat) chat = await createChat(activeProjectId, activeWorkspaceId, { cleanup: false, refresh: false }); const controller = new AbortController(); abortRef.current = controller; const accepted = () => { setDraft(""); setMessages((m) => [...m, { id: `optimistic-${Date.now()}`, chatId: chat.id, role: "user", content: trimmed, provider: null, metadata: attachments.length ? { attachments } : {}, createdAt: new Date().toISOString() }]); onAccepted?.(); }; await streamChatResponse(chat.id, `/api/chats/${chat.id}/messages`, turnBody(trimmed, chat, options.skillIds, attachments, useChatSettings ? chat : undefined), controller, "POST", true, accepted); } catch (error) { setBusy(false); setError(toErr(error)); throw error; } }
  ...
  async function streamChatResponse(chatId: string, url: string, body: unknown, controller: AbortController, method = "POST", notifyWhenDone = true, onAccepted?: () => void): Promise<void> {
