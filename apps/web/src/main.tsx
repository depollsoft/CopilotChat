...
function ProjectDefaultModelCard(p: { project: Project; models: ProviderStatus["models"]; onChange: (model: string) => void }) {
  const configuredModel = p.project.defaultModel;
  const modelAvailable = !configuredModel || p.models.some((model) => model.id === configuredModel);
  return <div className="project-summary-card project-model-card"><div className="card-h"><strong>Default model</strong><span className={`tag${modelAvailable ? "" : " warning"}`}>{modelAvailable ? configuredModel ? "Project" : "App default" : "Unavailable"}</span></div><p>{modelAvailable ? "New chats in this project start with this model unless the chat is changed later." : `${configuredModel} is no longer available. New chats use the app default until you choose a replacement.`}</p><select aria-label="Project default model" value={configuredModel ?? ""} onChange={(e) => p.onChange(e.currentTarget.value)}><option value="">Use app default</option>{configuredModel && !modelAvailable ? <option value={configuredModel}>{configuredModel} (unavailable)</option> : null}{p.models.map((model) => <option key={model.id} value={model.id}>{model.name || model.id}</option>)}</select></div>;
}
