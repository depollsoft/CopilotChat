...
function App(): React.ReactElement {
  ...
  const modelsDiscovered = provider.available && provider.models.length > 0;
  ...
  async function refreshModels(force = false): Promise<void> {
    ...
    const promise = (async () => {
      ...
      try {
        ...
        modelRefreshAttemptRef.current = nextProvider.available ? Date.now() : 0;
        setState((current) => current ? { ...current, provider: nextProvider } : current);
        if (force && nextProvider.available) { setError(null); setToast(`${nextProvider.models.length} models available`); }
        if (force && !nextProvider.available) setError(nextProvider.details || "Model discovery is unavailable.");
      } catch (e) {
    ...
    <main className="main">
      ...
      <div ref={scrollRef} className="scroll" onScroll={handleThreadScroll}>
        {activeProject && !selectedChat && state
          ? <ProjectHome project={activeProject} state={state} models={provider.models} modelsDiscovered={modelsDiscovered} chats={chats.filter((chat) => chat.projectId === activeProject.id)} onSelectChat={selectChat} onNewChat={() => void createChat(activeProject.id, activeWorkspaceId)} refresh={refreshState} showToast={setToast} />
...
function ProjectHome(p: { project: Project; state: AppState; models: ProviderStatus["models"]; modelsDiscovered: boolean; chats: Chat[]; onSelectChat: (id: string) => void; onNewChat: () => void; refresh: () => Promise<void>; showToast: (s: string) => void }) {
...
function ProjectDefaultModelCard(p: { project: Project; models: ProviderStatus["models"]; modelsDiscovered: boolean; onChange: (model: string) => void }) {
  ...
  return <div className="project-summary-card project-model-card"><div className="card-h"><strong>Default model</strong><span className={`tag${configuredModel && !modelAvailable ? " warning" : ""}`}>{status}</span></div><p>{description}</p><select aria-label="Project default model" value={configuredModel ?? ""} onChange={(e) => p.onChange(e.currentTarget.value)}><option value="">Use app default</option>{configuredModel && !modelAvailable ? <option value={configuredModel}>{configuredModel} ({modelUnverified ? "not verified" : "unavailable"})</option> : null}{p.modelsDiscovered ? p.models.map((model) => <option key={model.id} value={model.id}>{model.name || model.id}</option>) : null}</select></div>;
}
