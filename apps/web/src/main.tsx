...
import { ChatStreamRegistry } from "./chat-streams.js";
...
function App(): React.ReactElement {
  ...
  useEffect(() => {
    ...
    for (const chatId of previous) if (!runningChatIds.has(chatId) && chatId !== selectedChatIdRef.current) notify("CopilotChat", "Response ready");
  }, [runningChatIds]);
  ...
  async function streamChatResponse(chatId: string, url: string, body: unknown, controller: AbortController, method = "POST", notifyWhenDone = true, rethrowNetworkErrors = false, onAccepted?: () => void): Promise<void> {
    ...
    try {
      ...
      chatStreamsRef.current.end(chatId, controller);
      if (isVisibleChat(chatId)) setBusy(false);
    }
  }
  async function handleStreamEvent(chatId: string, event: SseEvent): Promise<boolean> {
    ...
    if (event.event === "usage") { if (visible) setLiveUsage(readChatUsage(event.data)); return false; }
    if (event.event === "pending") { if (visible) { setBusy(true); setPendingTurns(readPendingTurns((event.data as { pendingTurns?: unknown }).pendingTurns)); } return false; }
    if (event.event === "message") {
      await syncChatMessages(chatId);
      if (isVisibleChat(chatId)) { setStreamingText(""); setStreamingActivities([]); }
      return false;
    }
    if (event.event === "interaction") { if (visible) { setBusy(true); setPendingInteractions(readInteractions((event.data as { interactions?: unknown }).interactions)); } return false; }
    if (event.event === "activity") { if (visible) { setBusy(true); setStreamingActivities(readActivities((event.data as { activities?: unknown }).activities)); } return false; }
    if (event.event === "delta") { if (visible) { setBusy(true); setStreamingText((t) => t + ((event.data as { text?: string }).text ?? "")); } return false; }
    if (event.event === "artifact") { void refreshState(); if (visible) setToast("Artifact created"); return false; }
  ...
  async function stopActiveResponse(): Promise<void> {
    ...
    try {
      if (chatId) {
        ...
        removeRunningChat(chatId);
        if (isVisibleChat(chatId)) setMessages(await api<ChatMessage[]>(`/api/chats/${chatId}/messages`, {}, apiToken));
      }
    } catch (e) {
      setError(toErr(e));
    } finally {
