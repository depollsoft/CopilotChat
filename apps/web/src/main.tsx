...
function App(): React.ReactElement {
  ...
  async function renameChat(chat: Chat): Promise<void> { const title = window.prompt("Rename chat", chat.title); if (!title?.trim()) return; await api<Chat>(`/api/chats/${chat.id}`, { method: "PATCH", body: { title: title.trim() } }, apiToken); await refreshState(); }
  ...
  async function deleteChat(chat: Chat): Promise<void> { if (!window.confirm(`Delete "${chat.title}"?`)) return; await api<void>(`/api/chats/${chat.id}`, { method: "DELETE", raw: true }, apiToken); if (selectedChatId === chat.id) setSelectedChatId(null); await refreshState(); }
  ...
  async function createProjectFromSidebar(): Promise<void> { const name = window.prompt("New project name"); if (!name?.trim()) return; const project = await api<Project>("/api/projects", { method: "POST", body: { name: name.trim(), description: "", instructions: "", memory: "" } }, apiToken); selectProject(project.id); await refreshState(); }
  async function renameProject(project: Project): Promise<void> { const name = window.prompt("Rename project", project.name); if (!name?.trim()) return; await api<Project>(`/api/projects/${project.id}`, { method: "PATCH", body: { name: name.trim() } }, apiToken); await refreshState(); }
  async function deleteProject(project: Project): Promise<void> { if (!window.confirm(`Delete project "${project.name}"? Chats will move back to General.`)) return; await api<void>(`/api/projects/${project.id}`, { method: "DELETE", raw: true }, apiToken); if (activeProjectId === project.id) selectProject(null); await refreshState(); }
  ...
  async function clearAllData(): Promise<void> {
    if (!window.confirm("Clear all local CopilotChat data? This deletes chats, projects, artifacts, skills, MCP servers, workspaces, tool history, and isolated chat workspaces. Account/auth setup is kept.")) return;
