...
function AuthSetupCard(p:{provider:ProviderStatus}){const command="copilot login\n# or\ngh auth login\n# then restart the dev server\npnpm dev"; const infinite=p.provider.capabilities.includes("infinite-sessions"); const oauthFailure=p.provider.details.includes("GitHub OAuth token"); return <div className={`card auth-card${p.provider.available?" ready":" warning"}`}><div className="card-h"><strong>{p.provider.available?"Copilot is connected":"Connect Copilot"}</strong><span className={`tag${p.provider.available?" success":" warning"}`}>{p.provider.available?"ready":"setup needed"}</span></div><p className="section-help">{p.provider.available?`Dynamic model discovery is working${infinite?", and SDK infinite chats are enabled.":"."}`:oauthFailure?p.provider.details:"Your shell has no Copilot token, gh is not logged in, and the Copilot CLI is not logged in. Authenticate in the same terminal you use for pnpm dev."}</p>{!p.provider.available&&!oauthFailure?<pre className="command-block"><code>{command}</code></pre>:null}{oauthFailure?<div className="settings-actions"><a className="btn btn-primary" href="/api/auth/github/login">Sign in again</a></div>:null}</div>;}
